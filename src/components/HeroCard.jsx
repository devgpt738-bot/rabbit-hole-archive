...
export default function HeroCard({ onDismiss, onSelect }) {
        ...
        <div className="max-w-2xl mt-12 sm:mt-0 relative z-10 w-full min-w-0">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="font-sans text-xs sm:text-lg text-white/70 leading-relaxed max-w-xl mb-6 sm:mb-8 drop-shadow-[0_2px_5px_rgba(0,0,0,0.8)] break-words hyphens-auto pr-2 sm:pr-0 line-clamp-3 sm:line-clamp-none">
                {current.description}
              </p>
            </motion.div>
          </AnimatePresence>

          <button onClick={() => onSelect && onSelect(current)} className="flex items-center gap-2 sm:gap-3 px-5 py-2.5 sm:px-6 sm:py-3 bg-white text-black font-sans font-bold tracking-wider uppercase text-[10px] sm:text-sm rounded-full
